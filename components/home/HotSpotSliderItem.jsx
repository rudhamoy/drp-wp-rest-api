...
function HotSpotSliderItem({ category, title, image, slug, date }) {
    ...
    const safeTitle = DOMPurify.sanitize(title.substring(0, 60))
    ...
    return (
        // <Link href={`/single-news/${slug}`}>
            <div  className="flex flex-row items-center cursor-pointer rounded-md w-[400px] p-2 z-0 ">
                ...
                <div className="px-2 relative h-[80px] w-[300px]">
                    ...
                    <p className="blogTitle mb-[5px] text-[#bf912d] font-bold text-[10px] uppercase">{parseHTML(category.name)}</p>
                    </Link>
                ...
                <div className="w-[1px] rounded bg-[#afafaf61] h-[80px] mx-4"></div>
            </div>
        // </Link>
    )
}
